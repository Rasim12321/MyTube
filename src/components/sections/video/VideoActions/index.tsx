...
export function VideoActions({ video }: { video: ISingleVideoResponse }) {
  ...
  return (
    <div className='flex items-center gap-7'>
      <button className='flex items-center gap-1 opacity-80 transition-opacity hover:opacity-100'>
        <ListPlus size={20} />
        Save
      </button>
      <button
        className='text-primary flex items-center gap-1.5 opacity-80 transition-opacity hover:opacity-100'
        onClick={() => mutate()}
      >
        <Heart
          size={20}
          className={clsx('transition', {
            'fill-[var(--color-primary)]': isLikedLocal,
            'fill-transparent': !isLikedLocal,
          })}
        />{' '}
